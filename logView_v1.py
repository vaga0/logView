# logView_v1.py
# pyinstaller --noconsole --onefile --icon=logo-32.ico --add-data "D:\py_project\virtualEnv\v_name\Lib\site-packages\tkinterdnd2;tkinterdnd2/" logView_v1.py
...
class App:
	...
	def show_dashboard(self, response):
		...
		toggle_button = self.entities['_btn']['button_wrap'] = tk.Button(upper_frame_L2, text="換行", command=self.toggle_wrap)
		toggle_button.pack(side=tk.RIGHT, padx=5)  # 或其他的布局方式，根據你的需求
		...
		# 創建滾動條
		yscrollbar = tk.Scrollbar(main_frame, orient="vertical")
		yscrollbar.pack(side=tk.RIGHT, fill=tk.Y)

		xscrollbar = tk.Scrollbar(main_frame, orient="horizontal")
		xscrollbar.pack(side=tk.BOTTOM, fill=tk.X)

		wrap_setting = tk.NONE if not self.config["textarea_wrap"] else tk.WORD
		self.text_area = self.entities['_entry']['text_area'] = tk.Text(main_frame, wrap=wrap_setting, yscrollcommand=yscrollbar.set, xscrollcommand=xscrollbar.set)
		...
		self.text_area.insert(tk.END, "1. 拖曳檔案至此\n\n")
		self.text_area.insert(tk.END, "2. 開啟檔案\n\n")
		self.text_area.insert(tk.END, "3. 複製內容貼上\n\n")
	...
	def toggle_wrap(self):
		...
		if current_wrap == tk.NONE:	
			self.entities['_btn']['button_wrap'].config(text="不換行")
			self.text_area.config(wrap=tk.WORD) # 換行
		else:
			self.entities['_btn']['button_wrap'].config(text="換行")
			self.text_area.config(wrap=tk.NONE) # 不換行
